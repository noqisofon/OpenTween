...
namespace OpenTween
{
    public class PlixiMultimediaShareService : HttpConnectionOAuthEcho, IMultimediaShareService
    {
        private string[] picture_extensions_ = new string[] { ".jpg", ".jpeg", ".gif", ".png" };

        private const long max_filesize_ = 5 * 1024 * 1024;
        ...
        {
            if ( Array.IndexOf( picture_extensions_, ext.ToLower() ) > -1 )
                return true;
        ...
        {
            if ( this.CheckValidExtension( ext ) )
                return fileSize <= PlixiMultimediaShareService.max_filesize_;
        ...
        public PlixiMultimediaShareService( Twitter twitter )
        : base( new Uri( "http://api.twitter.com/" ), new Uri( "https://api.twitter.com/1/account/verify_credentials.json" ) )
        {
            this.twitter_ = twitter;
            base.Initialize( ApplicationSettings.TwitterConsumerKey, ApplicationSettings.TwitterConsumerSecret, twitter_.AccessToken, twitter_.AccessTokenSecret, "", "" );
        }
    }
}
