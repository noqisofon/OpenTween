...
namespace OpenTween
{
    public class TwitPicMultimediaShareService : HttpConnectionOAuthEcho, IMultimediaShareService
    {
        private string[] picture_extensions_ = new string[] { ".jpg", ".jpeg", ".gif", ".png" };

        private string[] multimedia_extensions_ = new string[] { ".avi", ".wmv", ".flv", ".m4v", ".mov", ".mp4", ".rm", ".mpeg", ".mpg", ".3gp", ".3g2" };

        private const long max_filesize_ = 10 * 1024 * 1024; // Image only
        ...
        public string Upload( ref string filePath, ref string message, long reply_to )
        {
            ...
            return twitter_.PostStatus( message, 0 );
        }
        ...
        public bool CheckValidExtension( string ext )
        {
            if ( Array.IndexOf( this.picture_extensions_, ext.ToLower() ) > -1 )
                return true;
            if ( Array.IndexOf( this.multimedia_extensions_, ext.ToLower() ) > -1 )
                return true;
        ...
        {
            return "Image Files(*" + string.Join( ";*", this.picture_extensions_ ) + ")|*" + string.Join( ";*", this.picture_extensions_ )
            + "|Videos(*" + string.Join( ";*", this.multimedia_extensions_ ) + ")|*" + string.Join( ";*", this.multimedia_extensions_ );
        }
        ...
        {
            if ( Array.IndexOf( this.picture_extensions_, ext.ToLower() ) > -1 )
                return fileSize <= TwitPicMultimediaShareService.max_filesize_;
            if ( Array.IndexOf( this.multimedia_extensions_, ext.ToLower() ) > -1 )
        ...
        public TwitPicMultimediaShareService( Twitter twitter )
        : base( new Uri( "http://api.twitter.com/" ), new Uri( "https://api.twitter.com/1/account/verify_credentials.json" ) )
        {
            this.twitter_ = twitter;
            this.Initialize( ApplicationSettings.TwitterConsumerKey, ApplicationSettings.TwitterConsumerSecret, twitter_.AccessToken, twitter_.AccessTokenSecret, "", "" );
        }
    }
}
